import type { ToolHandler } from "./types";
import {
  wordCounter,
  characterCounter,
  sentenceCounter,
  textReverser,
  textCaseConverter,
  removeExtraSpaces,
  randomStringGenerator,
  passwordGenerator,
  keywordDensityChecker,
  keywordFrequencyChecker,
  metaTagGenerator,
  metaDescriptionGenerator,
  titleTagGenerator,
  serpSnippetPreview,
  unixTimestampConverter,
  timestampGenerator,
  regexTester,
  regexEscapeTool,
  htmlEntityEncoder,
  javascriptFormatter,
  sqlFormatter,
  sqlMinifier,
  uuidGenerator,
  uuidValidator,
  findAndReplaceText,
  loremIpsumGenerator,
  textCleaner,
  duplicateWordFinder,
  palindromeChecker,
  jsonFormatter,
  jsonValidator,
  jsonMinifier,
  base64Encoder,
  base64Decoder,
  urlEncoder,
  urlDecoder,
  xmlFormatter,
  xmlValidator,
  cssFormatter,
  organizationSchemaGenerator,
  websiteSchemaGenerator,
  seoSlugGenerator,
  seoTitleChecker,
  metaDescriptionLengthChecker,
  schemaMarkupGenerator,
  faqSchemaGenerator,
  articleSchemaGenerator,
  breadcrumbSchemaGenerator,
  localBusinessSchemaGenerator,
  robotsTxtGenerator,
  sitemapGenerator,
  canonicalUrlGenerator,
  openGraphGenerator,
  twitterCardGenerator,
  headingStructureChecker,
  internalLinkChecker,
  keywordPlacementChecker,
  paragraphCounter,
  readingTimeCalculator,
  removeDuplicateLines,
  removeEmptyLines,
  lineCounter,
  wordFrequencyCounter,
  textSorter,
  htmlEntityDecoder,

  utmUrlBuilder,
  httpStatusCodeLookup,
  mimeTypeLookup,
  userAgentParser,
  domainNameParser,
  urlParser,
  urlCleaner,
  queryStringParser,
  queryStringBuilder,
  secretKeyGenerator,
  pinGenerator,
  imageAltTextGenerator,
  imageFilenameGenerator,
  imageDimensionsChecker,
  colorPicker,
  hexToRgbConverter,
  rgbToHexConverter,
  hslColorConverter,
  colorContrastChecker,
  youtubeTitleGenerator,
  youtubeDescriptionGenerator,
  youtubeTagGenerator,
  instagramCaptionGenerator,
  socialMediaCaptionGenerator,
  imageToBase64Converter,
  base64ToImageConverter,
  aspectRatioCalculator,
  imageUrlGenerator,
  hashtagGenerator,
  emailLinkGenerator,
  telLinkGenerator,
  anchorLinkGenerator,
  passwordStrengthChecker,
  randomNumberGenerator,
  hashGenerator,
  md5HashGenerator,
  sha256HashGenerator,
  sha512HashGenerator,
  hmacGenerator,
  callToActionGenerator,
  headlineGenerator,
  blogTitleGenerator,
  contentBriefGenerator,
  utmCampaignBuilder,
  csvToJsonConverter,
  jsonToCsvConverter,
  jsonToXmlConverter,
  xmlToJsonConverter,
  textToCsvConverter,
  csvColumnExtractor,
  numberToWordsConverter,
  wordsToNumberConverter,
  binaryToDecimalConverter,
  decimalToBinaryConverter,
  hexToDecimalConverter,
  decimalToHexConverter,
  romanNumeralConverter,
  percentageCalculator,
  percentageIncreaseCalculator,
  percentageDecreaseCalculator,
  averageCalculator,
  ratioCalculator,
  proportionCalculator,
  ageCalculator,
  dateDifferenceCalculator,
  timeDifferenceCalculator,
  characterLimitCalculator,
  compoundInterestCalculator,
  listRandomizer,
  checklistGenerator,
} from "./handlers";

const unavailableTool: ToolHandler = ({ text }) => {
  const clean = text.trim();

  if (!clean) {
    return {
      title: "No Input",
      content: "Please enter some content first.",
    };
  }

  const words = clean.split(/\s+/).filter(Boolean);

  return {
    title: "Tool Result",
    content: [
      `Words: ${words.length}`,
      `Characters: ${clean.length}`,
      "",
      "This tool is ready for its dedicated processor.",
    ].join("\n"),
  };
};

export const toolRegistry: Record<string, ToolHandler> = {
  "hash-generator": hashGenerator,
  "md5-hash-generator": md5HashGenerator,
  "sha256-hash-generator": sha256HashGenerator,
  "sha512-hash-generator": sha512HashGenerator,
  "hmac-generator": hmacGenerator,

  "email-link-generator": emailLinkGenerator,
  "tel-link-generator": telLinkGenerator,
  "anchor-link-generator": anchorLinkGenerator,
  "password-strength-checker": passwordStrengthChecker,
  "random-number-generator": randomNumberGenerator,

  "remove-duplicate-lines": removeDuplicateLines,
  "remove-empty-lines": removeEmptyLines,
  "line-counter": lineCounter,
  "word-frequency-counter": wordFrequencyCounter,
  "text-sorter": textSorter,


  "heading-structure-checker": headingStructureChecker,
  "internal-link-checker": internalLinkChecker,
  "keyword-placement-checker": keywordPlacementChecker,
  "paragraph-counter": paragraphCounter,
  "reading-time-calculator": readingTimeCalculator,


  "robots-txt-generator": robotsTxtGenerator,
  "sitemap-generator": sitemapGenerator,
  "canonical-url-generator": canonicalUrlGenerator,
  "open-graph-generator": openGraphGenerator,
  "twitter-card-generator": twitterCardGenerator,
  "find-and-replace-text": findAndReplaceText,
  "lorem-ipsum-generator": loremIpsumGenerator,
  "text-cleaner": textCleaner,
  "duplicate-word-finder": duplicateWordFinder,
  "palindrome-checker": palindromeChecker,
  "json-formatter": jsonFormatter,
  "json-validator": jsonValidator,
  "json-minifier": jsonMinifier,
  "base64-encoder": base64Encoder,
  "base64-decoder": base64Decoder,
  "url-encoder": urlEncoder,
  "url-decoder": urlDecoder,
  "xml-formatter": xmlFormatter,
  "xml-validator": xmlValidator,
  "css-formatter": cssFormatter,
  "organization-schema-generator": organizationSchemaGenerator,
  "website-schema-generator": websiteSchemaGenerator,
  "seo-slug-generator": seoSlugGenerator,
  "seo-title-checker": seoTitleChecker,
  "meta-description-length-checker": metaDescriptionLengthChecker,
  "schema-markup-generator": schemaMarkupGenerator,
  "faq-schema-generator": faqSchemaGenerator,
  "article-schema-generator": articleSchemaGenerator,
  "breadcrumb-schema-generator": breadcrumbSchemaGenerator,
  "local-business-schema-generator": localBusinessSchemaGenerator,

  "keyword-frequency-checker": keywordFrequencyChecker,
  "meta-tag-generator": metaTagGenerator,
  "meta-description-generator": metaDescriptionGenerator,
  "title-tag-generator": titleTagGenerator,
  "serp-snippet-preview": serpSnippetPreview,
  "unix-timestamp-converter": unixTimestampConverter,
  "timestamp-generator": timestampGenerator,
  "regex-tester": regexTester,
  "regex-escape-tool": regexEscapeTool,
  "html-entity-encoder": htmlEntityEncoder,
  "html-entity-decoder": htmlEntityDecoder,

  "utm-url-builder": utmUrlBuilder,
  "http-status-code-lookup": httpStatusCodeLookup,
  "mime-type-lookup": mimeTypeLookup,
  "user-agent-parser": userAgentParser,
  "domain-name-parser": domainNameParser,
  "url-parser": urlParser,
  "url-cleaner": urlCleaner,
  "query-string-parser": queryStringParser,
  "query-string-builder": queryStringBuilder,
  "secret-key-generator": secretKeyGenerator,
  "pin-generator": pinGenerator,
  "image-alt-text-generator": imageAltTextGenerator,
  "image-filename-generator": imageFilenameGenerator,
  "image-dimensions-checker": imageDimensionsChecker,
  "color-picker": colorPicker,
  "hex-to-rgb-converter": hexToRgbConverter,
  "rgb-to-hex-converter": rgbToHexConverter,
  "hsl-color-converter": hslColorConverter,
  "color-contrast-checker": colorContrastChecker,
  "youtube-title-generator": youtubeTitleGenerator,
  "youtube-description-generator": youtubeDescriptionGenerator,
  "youtube-tag-generator": youtubeTagGenerator,
  "instagram-caption-generator": instagramCaptionGenerator,
  "social-media-caption-generator": socialMediaCaptionGenerator,
  "image-to-base64-converter": imageToBase64Converter,
  "base64-to-image-converter": base64ToImageConverter,
  "aspect-ratio-calculator": aspectRatioCalculator,
  "image-url-generator": imageUrlGenerator,
  "hashtag-generator": hashtagGenerator,
  "javascript-formatter": javascriptFormatter,
  "sql-formatter": sqlFormatter,
  "sql-minifier": sqlMinifier,
  "uuid-generator": uuidGenerator,
  "uuid-validator": uuidValidator,

  "word-counter": wordCounter,
  "character-counter": characterCounter,
  "sentence-counter": sentenceCounter,
  "text-reverser": textReverser,
  "text-case-converter": textCaseConverter,
  "remove-extra-spaces": removeExtraSpaces,
  "random-string-generator": randomStringGenerator,
  "password-generator": passwordGenerator,
  "keyword-density-checker": keywordDensityChecker,
  "text-to-csv-converter": textToCsvConverter,
  "xml-to-json-converter": xmlToJsonConverter,
  "json-to-xml-converter": jsonToXmlConverter,
  "json-to-csv-converter": jsonToCsvConverter,
  "csv-to-json-converter": csvToJsonConverter,
  "utm-campaign-builder": utmCampaignBuilder,
  "content-brief-generator": contentBriefGenerator,
  "blog-title-generator": blogTitleGenerator,
  "headline-generator": headlineGenerator,
  "csv-column-extractor": csvColumnExtractor,
  "number-to-words-converter": numberToWordsConverter,
  "words-to-number-converter": wordsToNumberConverter,
  "binary-to-decimal-converter": binaryToDecimalConverter,
  "decimal-to-binary-converter": decimalToBinaryConverter,
  "call-to-action-generator": callToActionGenerator,
  "hex-to-decimal-converter": hexToDecimalConverter,
  "decimal-to-hex-converter": decimalToHexConverter,
  "roman-numeral-converter": romanNumeralConverter,
  "percentage-calculator": percentageCalculator,
  "percentage-increase-calculator": percentageIncreaseCalculator,
  "percentage-decrease-calculator": percentageDecreaseCalculator,
  "average-calculator": averageCalculator,
  "ratio-calculator": ratioCalculator,
  "proportion-calculator": proportionCalculator,
  "age-calculator": ageCalculator,
  "date-difference-calculator": dateDifferenceCalculator,
  "time-difference-calculator": timeDifferenceCalculator,
  "character-limit-calculator": characterLimitCalculator,
  "compound-interest-calculator": compoundInterestCalculator,
  "list-randomizer": listRandomizer,
  "checklist-generator": checklistGenerator,
  /*
   * Additional dedicated handlers will be added incrementally.
   */
};

export function getToolHandler(slug: string): ToolHandler {
  return toolRegistry[slug] || unavailableTool;
}


